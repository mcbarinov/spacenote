# /start-concepts

Start a new concepts session for brainstorming domain ideas and data structures.

## Description

This command initializes a session focused on understanding project concepts and brainstorming new ideas:

- `README.md` - Monorepo overview and project structure
- `docs/concepts.md` - Core domain model (Space, Note, Fields, Users, Comments)
- `docs/fields.md` - Field types, options, and VALUE_MAPS reference
- `docs/space-examples.md` - Real-world space configuration examples
- `docs/ideas.md` - Future improvement ideas and planned features
- `CLAUDE.md` - Development guidelines and project rules

**Focus**: High-level domain concepts and data structures, not implementation details.

## Usage

```
/start-concepts
```

## Actions

1. Read `README.md` to understand the monorepo structure
2. Read `docs/concepts.md` to understand the core domain model
3. Read `docs/fields.md` for field types and options
4. Read `docs/space-examples.md` for use case examples
5. Read `docs/ideas.md` for future improvement ideas
6. Read `CLAUDE.md` for project guidelines
7. Confirm readiness to brainstorm

## Response

After reading all files, confirm with:
"I've read the project concepts. Ready to brainstorm!"

ultrathink
